    ...
    public init(url: URL? = nil, encoding: String.Encoding? = nil) {
        DispatchQueue.global(qos: .background).async {
            do {
                ...
                self.groups = VTTParser.parseSubRip(string) ?? []
            } catch {
                print("| [Error] failed to load in subtitle")
            }
        }
    }
            ...
            do {
                if let encoding = encoding {
                    ...
                    if subTitle.contains("WEBVTT") {
                        ...
                        self.groups = VTTParser.parseSubRip(scr) ?? []
                    } else {
                        self.groups = VTTParser.parseSubRip(subTitle) ?? []
                    }
                } else {
